...
class CostComposite(Cost):
    ...
    def eval(self, trajs, trajs_interpolated=None, return_invidual_costs_and_weights=False, aggregate=True, **kwargs):
        ...
        if not return_invidual_costs_and_weights:
            ...
            for cost, weight_cost in zip(self.cost_l, self.weight_cost_l):
                if trajs_interpolated is not None:
                    ...
                    trajs_tmp = trajs
                cost_tmp = weight_cost * cost(trajs_tmp, q_pos=q_pos, q_vel=q_vel, H_positions=H_positions, aggregate=aggregate, **kwargs)
                cost_total += cost_tmp
            return cost_total
        else:
            cost_l = []
            for cost in self.cost_l:
                ...
                cost_tmp = cost(trajs_tmp, q_pos=q_pos, q_vel=q_vel, H_positions=H_positions, aggregate=aggregate, **kwargs)
...
class CostCollision(Cost):
    ...
    def eval(self, trajs, q_pos=None, q_vel=None, H_positions=None, aggregate=True, **observation):
        costs = 0
        if self.field is not None:
            ...
            obst_costs = w_mat * err_obst
            costs = obst_costs

            if aggregate:
                return costs.sum(1)
            
            costs = torch.cat([torch.zeros((costs.shape[0], 1)).to(**self.tensor_args), costs], dim=1)
            return costs
...
class CostGPTrajectory(Cost):
    ...
    def eval(self, trajs, aggregate=True, **observation):
        # trajs = trajs.reshape(-1, self.n_support_points, self.dim)
        ...
        w_mat = self.gp_prior.Q_inv[0]  # repeated Q_inv
        w_mat = w_mat.reshape(1, 1, self.dim, self.dim)
        gp_costs = (err_gp.transpose(2, 3) @ w_mat @ err_gp).squeeze()

        if aggregate:
            return gp_costs.sum(1)
        return torch.cat([torch.zeros((gp_costs.shape[0], 1)).to(**self.tensor_args), gp_costs], dim=1)

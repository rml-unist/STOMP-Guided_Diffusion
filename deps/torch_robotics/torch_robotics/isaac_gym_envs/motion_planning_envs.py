...
# from isaacgym.python.isaacgym.torch_utils import to_torch
from torch_robotics.environments.env_table_shelf import EnvTableShelf
from torch_robotics.environments.env_table_objects import EnvTableObject
...
from torch_robotics.torch_kinematics_tree.models.robots import modify_franka_panda_urdf_grasped_object
...
def set_position_and_orientation(center, obj_pos, obj_ori):
    ...
    obj_pose.p = gymapi.Vec3(*(rotate_vector_by_quat(center, obj_ori) + obj_pos))
...
class ViewerRecorder:
    ...
    def make_video(
        ...
        if make_gif:
            ...
            video.write_gif(gif_path, fps=video.fps, loop=1)
...
class PandaMotionPlanningIsaacGymEnv:
    ...
    def __init__(self, env, robot, task,
        ...
        if self.robot.grasped_object is not None:
            ...
            franka_asset_file_abs_path_new = modify_franka_panda_urdf_grasped_object(
        ...
        self.obj_num = len(object_fixed_assets_l) + len(object_extra_assets_l)
        ...
        asset_options.fix_base_link = True # if isinstance(self.robot, RobotPanda) else False
        asset_options.disable_gravity = True # if isinstance(self.robot, RobotPanda) else False
        ...
        if self.controller_type == 'position':
            franka_dof_props["driveMode"][:self.franka_num_dofs-2].fill(gymapi.DOF_MODE_POS)
            franka_dof_props["stiffness"][:self.franka_num_dofs-2].fill(400.0)
            # franka_dof_props["damping"][:7].fill(40.0)
            # franka_dof_props["stiffness"][:7] = np.array([400.0, 300.0, 300.0, 200.0, 150.0, 100.0, 50.0])
            franka_dof_props["damping"][:self.franka_num_dofs-2] = 2. * np.sqrt(franka_dof_props["stiffness"][:self.franka_num_dofs-2])
        elif self.controller_type == 'velocity':
            franka_dof_props["driveMode"][:self.franka_num_dofs-2].fill(gymapi.DOF_MODE_VEL)
            franka_dof_props["stiffness"][:self.franka_num_dofs-2].fill(0.0)
            franka_dof_props["damping"][:self.franka_num_dofs-2].fill(600.0)
        else:
            raise NotImplementedError

        # use position control for grippers
        franka_dof_props["driveMode"][self.franka_num_dofs-2:].fill(gymapi.DOF_MODE_POS)
        franka_dof_props["stiffness"][self.franka_num_dofs-2:].fill(800.0)
        franka_dof_props["damping"][self.franka_num_dofs-2:].fill(40.0)

        # grippers open
        self.default_dof_pos[self.franka_num_dofs-2:] = self.franka_upper_limits[self.franka_num_dofs-2:]
        ...
        for i in range(self.num_envs):
            ...
            if self.show_goal_configuration and i == self.num_envs - 1:
                color = gymapi.Vec3(0, 0.5, 0) if goal_as_start_vis else gymapi.Vec3(128/255., 0., 128/255.)  # purple
        ...
        if "Table" in type(self.env).__name__:
            cam_pos = gymapi.Vec3(2.05, 0, 1.25)
        ...
        self.dof_pos = self.dof_states[:, 0].view(self.num_envs, self.franka_num_dofs, 1)
        self.dof_vel = self.dof_states[:, 1].view(self.num_envs, self.franka_num_dofs, 1)
    ...
    def reset(self, start_joint_positions=None, goal_joint_position=None):
        ...
        if start_joint_positions is None:
            ...
            start_joint_positions[..., :self.franka_num_dofs - 2] = to_torch(self.default_dof_pos[:self.franka_num_dofs - 2], **self.tensor_args)
        ...
        if self.show_goal_configuration:
            dof_pos_tensor[:-1, :self.franka_num_dofs - 2] = start_joint_positions
            assert goal_joint_position is not None
            self.goal_joint_position = goal_joint_position
            dof_pos_tensor[-1, :self.franka_num_dofs - 2] = goal_joint_position
        else:
            dof_pos_tensor[..., :self.franka_num_dofs - 2] = start_joint_positions

        # grippers open
        dof_pos_tensor[..., self.franka_num_dofs - 2:] = to_torch(self.franka_upper_limits, **self.tensor_args)[None, self.franka_num_dofs - 2:]
        ...
        joint_states_curr = gymtorch.wrap_tensor(self.gym.acquire_dof_state_tensor(self.sim)).view(self.num_envs, self.franka_num_dofs, 2)
        if self.show_goal_configuration:
    ...
    def step(self, actions, visualize=True, render_viewer_camera=False):
        ...
        if self.show_goal_configuration:
            action_dof[:-1, :self.franka_num_dofs - 2] = actions[..., :self.franka_num_dofs - 2]
            if self.controller_type == 'position':
                action_dof[-1, :self.franka_num_dofs - 2] = self.goal_joint_position
            elif self.controller_type == 'velocity':
                action_dof[-1, :self.franka_num_dofs - 2] = torch.zeros_like(self.goal_joint_position)
            else:
                raise NotImplementedError
        else:
            action_dof[..., :self.franka_num_dofs - 2] = actions[..., :self.franka_num_dofs - 2]

        # gripper is open
        action_dof[..., self.franka_num_dofs - 2:self.franka_num_dofs] = torch.Tensor([[0.04, 0.04]] * self.num_envs)
        ...
        if visualize:
            ...
            for k, (env, franka_handle) in enumerate(zip(envs, self.franka_handles)):
                ...
                if self.color_robots_in_collision:
                ...
                if self.show_collision_spheres:
                    ...
                    joint_pos_curr = to_torch(joint_state_curr['pos'][:self.franka_num_dofs - 2], device='cuda') # force the device to be gpu
                    fk_link_pos = self.robot.fk_map_collision(joint_pos_curr).to(**self.tensor_args)
...
class MotionPlanningController:
    ...
    def run_trajectories(
        ...
        for i, actions in enumerate(trajectories_copy):
            ...
            # error at this point signifies collision at the goal point
        ...
        collision_free_idx = [idx for idx in list(range(B)) if idx not in envs_with_robot_in_contact_unique]
        print(f'trajectories free in Isaac: {len(collision_free_idx)}/{B}')
        return collision_free_idx
    
    def render_trajectory_positions(self, trajectory, color=gymapi.Vec3(1, 0, 0), 
                                    n_first_steps=10, save_path=None):
        assert trajectory.ndim == 2

        joint_states = self.mp_env.reset(start_joint_positions=trajectory[0].unsqueeze(0), goal_joint_position=trajectory[0])
        ...
        for _ in range(n_first_steps):
            if self.mp_env.check_viewer_has_closed():
                break
            if self.mp_env.controller_type == 'position':
                _, _ = self.mp_env.step(joint_positions_start, visualize=False)
            elif self.mp_env.controller_type == 'velocity':
                _, _ = self.mp_env.step(joint_velocities_zero, visualize=False)
            else:
                raise NotImplementedError
        ...
        for action in trajectory:
            ...
            if self.mp_env.check_viewer_has_closed():
                break
            joint_states, _ = self.mp_env.step(action, visualize=False)
            # End-effector frame
            body_dict = self.mp_env.gym.get_actor_rigid_body_dict(self.mp_env.envs[0], self.mp_env.franka_handles[0])
            props = self.mp_env.gym.get_actor_rigid_body_states(self.mp_env.envs[0], self.mp_env.franka_handles[0], gymapi.STATE_POS)
            ee_pose = props['pose'][:][body_dict[self.mp_env.franka_hand]]
            ee_transform = gymapi.Transform(p=gymapi.Vec3(*ee_pose[0]), r=gymapi.Quat(*ee_pose[1]))
            # reference frame
            gymutil.draw_lines(self.mp_env.axes_geom, self.mp_env.gym, self.mp_env.viewer, self.mp_env.envs[0], ee_transform)
            end_effector_positions.append(list(ee_pose[0]))

        # Debug lines for end-effector trajectory
        color = np.array([[1, 0, 0]], dtype=np.float32)
        for i in range(1, len(end_effector_positions)):
            start_pos = end_effector_positions[i-1]
            end_pos = end_effector_positions[i]
            line = np.array([start_pos + end_pos], dtype=np.float32)
            self.mp_env.gym.add_lines(self.mp_env.viewer, self.mp_env.envs[0], 1, line, color)

        # Step through rendering to see visualization
        for _ in range(100):
            self.mp_env.gym.simulate(self.mp_env.sim)
            self.mp_env.gym.fetch_results(self.mp_env.sim, True)
            self.mp_env.gym.step_graphics(self.mp_env.sim)
            self.mp_env.gym.draw_viewer(self.mp_env.viewer, self.mp_env.sim, False)
            self.mp_env.gym.sync_frame_time(self.mp_env.sim)
        
        if save_path:
            self.mp_env.gym.render_all_camera_sensors(self.mp_env.sim)
            viewer_img = self.mp_env.gym.get_camera_image(self.mp_env.sim, self.mp_env.middle_env, self.mp_env.viewer_camera_handle, gymapi.IMAGE_COLOR)
            viewer_img = viewer_img.reshape(viewer_img.shape[0], -1, 4)[..., :3]  # get RGB part
            viewer_img = cv2.cvtColor(viewer_img, cv2.COLOR_BGR2RGB)
            cv2.imwrite(save_path, viewer_img)

...
class RobotPanda(RobotBase):
    ...
    def render(self, ax, q=None, color='blue', arrow_length=0.15, arrow_alpha=1.0, arrow_linewidth=2.0,
               draw_links_spheres=False, draw_body=True, return_pos=False, **kwargs):
        # draw skeleton
        if draw_body:
        ...
        if return_pos:
            return to_numpy(frame_EE.translation).squeeze()
        ...
        if trajs is not None:
            ...
            for traj, color in zip(trajs_pos, colors):
                for t in range(traj.shape[0]):
                    q = traj[t]
                    pos = self.render(ax, q, color, **kwargs, arrow_length=0.1, arrow_alpha=0.5, arrow_linewidth=1., 
                                      draw_body=False, return_pos=True)
                    pos_3ds.append(pos)
            ...
            pos_3ds_np = np.array(pos_3ds)
            ax.plot(pos_3ds_np[:, 0], pos_3ds_np[:, 1], zs=pos_3ds_np[:, 2], linewidth=2, c='black')

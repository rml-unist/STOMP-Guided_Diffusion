...
class URDFRobotModel(RobotModel):
    def __init__(self, model_path, device="cpu"):
        super().__init__(model_path, device=device)
        print("In URDF robot model: ", self.model_path)
        lock_path = f"{self.model_path}.lock"  # Creates a lock file alongside the URDF file
        ...
        while retries > 0:
            try:
                ...
                print(f"Failed to load URDF, retrying... ({3 - retries + 1}/3)")
                retries -= 1

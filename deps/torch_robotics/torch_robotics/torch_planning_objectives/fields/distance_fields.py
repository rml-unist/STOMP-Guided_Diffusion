...
def interpolate_points_v1(points, num_interpolated_points):
    """
    Optimized interpolation for 3D and 4D tensors.
    Interpolates along the traj_len dimension with minimal tensor reshaping.
    """
    if points.dim() == 3:
        # For 3D case: [batch, traj_len, state_dim]
        # Interpolate directly without additional transposing back and forth
        points = Functional.interpolate(
            points.permute(0, 2, 1), size=num_interpolated_points, mode='linear', align_corners=True
        ).permute(0, 2, 1)

    elif points.dim() == 4:
        # For 4D case: [sample_num, batch, traj_len, state_dim]
        sample_num, batch, traj_len, state_dim = points.shape
        # Reshape and interpolate directly without repeated transposing
        points = Functional.interpolate(
            points.reshape(sample_num * batch, state_dim, traj_len), size=num_interpolated_points, mode='linear', align_corners=True
        ).view(sample_num, batch, state_dim, num_interpolated_points).permute(0, 1, 3, 2)
    else:

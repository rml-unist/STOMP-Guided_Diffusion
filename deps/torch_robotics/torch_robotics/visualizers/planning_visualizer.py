...
class PlanningVisualizer:
    ...
    def render_robot_trajectories(self, fig=None, ax=None, render_planner=False, trajs=None, traj_best=None, **kwargs):
        ...
        if trajs is not None:
            trajs_coll, trajs_coll_idxs, trajs_free, trajs_free_idxs, _ = self.task.get_trajs_collision_and_free(trajs, return_indices=True)

            kwargs['colors'] = [self.colors['collision']] * len(trajs_coll_idxs)
            self.robot.render_trajectories(ax, trajs=trajs_coll, **kwargs)
            kwargs['colors'] = [self.colors['free']] * len(trajs_free_idxs)
            self.robot.render_trajectories(ax, trajs=trajs_free, **kwargs)

        if traj_best is not None:
            kwargs['colors'] = [self.best_color]
    ...
    def animate_robot_trajectories(
        ...
        # show how a robot follows planned trajectories for each timestep
        if trajs is None:
            return
    ...
    def animate_opt_iters_robots(
        ...
        def animate_fn(i):
            ax.clear()
            ax.set_title(f"iter: {idxs[i]+1}/{S}")
            self.render_robot_trajectories(
    ...
    def plot_opt_iters_robot(
            self, fig=None, 
            trajs=None, 
            start_state=None, 
            goal_state=None, 
            draw_steps=None, 
            plot_filepath='./plot.png', 
            **kwargs
    ):
        if draw_steps is None or not isinstance(draw_steps, list) or trajs is None:
            return
        
        if fig is None:
            fig, ax = create_fig_and_axes(self.env.dim)
            ax.remove()
        
        print("Creating a figure...")
        S, B, H, D = trajs.shape

        draw_num = len(draw_steps)
        fig.set_size_inches(10*draw_num, 10, forward=True)

        for i, draw_step in enumerate(draw_steps):
            ax = fig.add_subplot(1, draw_num, i+1)
            ax.set_title(f"step: {draw_step+1}/{S}")

            self.render_robot_trajectories(
                fig=fig, ax=ax, trajs=trajs[draw_step],
                start_state=start_state, goal_state=goal_state, **kwargs
            )
        print("...finished Creating a figure...")

        print("Saving a figure...")
        fig.savefig(plot_filepath)
        print("...finished Saving a figure...")
    ...
    def plot_joint_space_state_trajectories(
        ...
        for i, ax in enumerate(axs):
            ...
            if traj_best is not None:
                ...
                plot_multiline(ax[0], timesteps, traj_best_pos_np[..., i].reshape(1, -1), color=self.best_color, **kwargs)
                plot_multiline(ax[1], timesteps, traj_best_vel_np[..., i].reshape(1, -1), color=self.best_color, **kwargs)
    ...
    def animate_opt_iters_joint_space_state(
        ...
        def animate_fn(i):
            ...
            fig.suptitle(f"iter: {idxs[i]+1}/{S}")
...
def create_animation_video(fig, animate_fn, anim_time=5, n_frames=100, video_filepath='video.gif', **kwargs):
    ...
    print(f'{str_start}...')
    ani.save(os.path.join(video_filepath), fps=max(1, int(n_frames / anim_time)), dpi=90, extra_args=['-loop', '-1'])
    print(f'...finished {str_start}')
